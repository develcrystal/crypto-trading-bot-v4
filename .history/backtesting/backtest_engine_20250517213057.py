        ...
        for i in range(30, len(data)):  # Start bei 30, um genügend Daten für Indikatoren zu haben
            ...
            if position is None:
                ...
                if signal['action'] in ['buy', 'sell']:
                    # Position eröffnen
                    position = 'long' if signal['action'] == 'buy' else 'short'
                    
                    # Einstiegspreis mit Slippage berechnen
                    entry_price = self._apply_slippage(current_close, signal['action'])
                    ...
                    # Standardwerte, falls nicht im Signal enthalten
                    if stop_loss == 0.0:
                    ...
                    logger.info(f"{signal['action'].upper()} Signal: Eintritt bei ${entry_price:.2f}, "
