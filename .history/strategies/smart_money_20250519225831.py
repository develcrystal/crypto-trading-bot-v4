...
class SmartMoneyStrategy:
    ...
    def _determine_session(self, df: pd.DataFrame) -> pd.Series:
        """
        ...
        sessions = pd.Series(['unknown'] * len(df), index=df.index)
        ...
        # Bestimme die Session für jeden Zeitstempel
        sessions[(utc_index.hour >= asian_start) & (utc_index.hour < asian_end)] = 'asian'
        sessions[(utc_index.hour >= london_start) & (utc_index.hour < london_end)] = 'london'
        sessions[(utc_index.hour >= new_york_start) & (utc_index.hour < new_york_end)] = 'new_york'
    ...
    def _calculate_base_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Args:
            df: Ein Pandas DataFrame mit Marktdaten (OHLCV).

        Returns:
            DataFrame mit hinzugefügten Basisindikatoren.
        """
        if len(df) < max(self.rsi_period, self.macd_slow, self.atr_period):
            logging.warning("Nicht genügend Daten für Basisindikator-Berechnung.")
            # Füge Dummy-Werte für die Ergebnis-Spalten hinzu
            df['rsi'] = np.nan
            df['ema_fast'] = np.nan
            df['ema_slow'] = np.nan
        ...
        logging.debug("Basisindikator-Berechnung abgeschlossen.")
        return df
