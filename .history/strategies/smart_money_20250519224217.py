    ...
    def _add_pattern_recognition(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['shooting_star'] = (
            (df['body_to_range'] < 0.3) &  # Small body
            (df['upper_shadow'] > df['body'] * 2) &  # Long upper shadow
            (df['lower_shadow'] < df['body'])  # Small lower shadow
        )

        # 5. Hammer (small body at bottom, long lower shadow, small upper shadow)
        df['hammer'] = (
            (df['body_to_range'] < 0.3) &  # Small body
            (df['lower_shadow'] > df['body'] * 2) &  # Long lower shadow
            (df['upper_shadow'] < df['body'])  # Small upper shadow
        )