    ...
    try:
        ...
        if hasattr(config, 'BYBIT_API_KEY') and config.BYBIT_API_KEY:
            exchange_api = BybitAPI(
                ...
                testnet=True  # Angenommen, Testnet ist standardmäßig aktiviert
            )
        ...
        full_config = vars(config).copy()
        full_config.update(strategy_params)
        ...
        if args.mode == 'basic':
            ...
            # Konvertiere Start- und Enddatum in Millisekunden
            # Millisekunden-Zeitstempel für die API erstellen
            start_date_ms = int(datetime.strptime(args.start_date, '%Y-%m-%d').timestamp() * 1000)
            end_date_ms = int(datetime.now().timestamp() * 1000) if args.end_date is None else int(datetime.strptime(args.end_date, '%Y-%m-%d').timestamp() * 1000)
            
            logger.info(f"Zeitraum für Backtest: {args.start_date} bis {args.end_date or 'jetzt'} (Timestamps: {start_date_ms} - {end_date_ms})")
