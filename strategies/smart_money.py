...
class SmartMoneyStrategy:
    ...
    def generate_signal(self, data: pd.DataFrame, current_position: str = None) -> Tuple[str, float, float, Dict]:
        ...
        # Nur prüfen, wenn es aktivierte Filter gibt
    ...
    def _add_pattern_recognition(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Kombiniere bullische und bärische Muster
        df['bullish_pattern'] = (
            df['bullish_engulfing'] | 
            df['hammer'] | 
            df['morning_star']  # Auch wenn morning_star zurzeit False ist
        )
        
        df['bearish_pattern'] = (
            df['bearish_engulfing'] | 
            df['shooting_star'] | 
            df['evening_star']  # Auch wenn evening_star zurzeit False ist
        )
